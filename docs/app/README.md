...
4. **通用系统定位**:App 与 mica-admin 一致,定位为通用后台系统的移动端,**不内置即时通讯**等业务专属能力。
...
| 维度 | 内容 |
|---|---|
| **一句话定位** | 让 mica-admin 体系内的员工与管理员,在手机上完成 **消息、查资料、轻量办公** |
| **目标用户** | 内部员工(查消息、找同事、改资料) + 各级管理员(看监控日志、查公告) |
| **核心价值** | 复用 mica-admin 已有的 RBAC、JWT、消息、文件存储,**零后端改造搭出 App** |
| **不做什么** | 即时通讯、视频会议、考勤打卡 —— 这些属于业务专属模块,留到二次开发或后续评估 |
...
| 通知公告 | `SysNoticeController` | `/api/system/notice/feed` `/{id}` |
...
> 「消息」Tab 内容:**公告**(`sys_notice` Feed) + **系统消息**(`sys_user_message`)两个子 Tab。

**4 个 Tab 的真实后端对应**:

| Tab | 含子功能 | 后端对应接口 |
|---|---|---|
| 工作台 | 问候/系统未读统计/最新公告/快捷入口 | `/api/auth/info` + `/api/system/user/message/unread` + `/api/system/notice/feed` |
| 消息 | 公告 Feed + 系统消息(已读/未读) | `/api/system/notice/feed` + `/api/system/user/message/*` |
...
| 图表 | uCharts | 跨端、轻量 |
| 推送 | 系统级推送(uniPush/个推)按需接入 | 见 [extension.md §6](./extension.md#6-实时推送) |
...
| 2 | 工作台 | ✅ | 聚合现有接口 | - |
| 3 | 消息中心(公告 + 系统消息) | ✅ | `/api/system/user/message/*` + `/api/system/notice/feed` | 点击跳转业务单据 |
...
- 最近一次重大更新:**IM 模块下线**(v1.0 起 mica-admin 定位为通用后台系统)。